...
@export var transform_threshold_meters: float = 5.0
...
func _process(delta):
	if is_instance_valid(player_ref):
		...
		var min_dist_px = 999999.0
		if nearest_enemy: min_dist_px = min(min_dist_px, player_ref.global_position.distance_to(nearest_enemy.global_position))
		if nearest_friend: min_dist_px = min(min_dist_px, player_ref.global_position.distance_to(nearest_friend.global_position))
		
		if ui and ui.has_method("update_transform_info"):
			ui.update_transform_info(min_dist_px / PIXELS_PER_METER)
...
func _process_trail_transformation():
	...
	var threshold_px = transform_threshold_meters * PIXELS_PER_METER
	...
	for i in range(points.size()):
		...
		if nearest_enemy and pos.distance_to(nearest_enemy.global_position) < threshold_px:
		...
		if nearest_friend and pos.distance_to(nearest_friend.global_position) < threshold_px:
